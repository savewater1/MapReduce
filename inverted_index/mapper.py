# -*- coding: utf-8 -*-
"""
Created on Fri Nov  8 18:31:31 2019
...
def append_key(sock, key, value):
    """
    Sends the append command to server
    """
    msg = pickle.dumps(('apd', key, len(value), value))
    send_msg(sock, msg)
    msg = recv_msg(sock)
    if msg == False:
...
if __name__ == "__main__":
    try:
        ...
        input_data = [(doc, content.split()) for doc, content in input_data]
        output_data = [(w, (doc, content.count(w))) for doc, content in input_data for w in set(content)]
