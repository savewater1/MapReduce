...
# =============================================================================
            ...
            if names:
                res = create_workers(names, self.gcloud["project"], self.gcloud["zone"], self.gcloud["network"])
                logging.debug(res)
    ...
    def runmapred(self): 
        try:
            ...
            if del_workers:
                ...
                res = delete_workers(del_workers, self.gcloud["project"], self.gcloud["zone"])
                logging.debug(res)
            if new_workers:
...
def create_workers(names, project, zone, network):
    ...
    """
    cmd = ["gcloud", "compute", "instances", "create", *names, "--project", project, "--zone", zone, "--no-service-account", "--no-scopes", "--network", network, "--metadata=enable-oslogin=TRUE", "--quiet"]
...
def delete_workers(names, project, zone):
    """
    Delete gcloud compute instances.
    """
    cmd = ["gcloud", "compute", "instances", "delete", *names, "--project", project, "--zone", zone, "--quiet"]
    proc = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    proc.wait()
    out, err = proc.communicate()
    if out:
